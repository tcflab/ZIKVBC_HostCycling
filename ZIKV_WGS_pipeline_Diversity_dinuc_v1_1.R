...
list.of.packages <- c("diverse","plyr","dplyr","splitstackshape","RcppRoll")
...
repfull<-paste("rep",rep,sep="")
sampledir<-paste(name,repfull,sep="_")
sampledir2<-paste(sampledir,"_v3.1",sep="")
outputdir<-paste(homedir,sampledir2,sep="/")
samplename<-paste(name,rep,sep="_")
...
table<-paste("ntcounts_consensus_",samplename,sep="")
table<-paste(table,".csv",sep="")
table<-paste(outputdir,table,sep="/")
ntcounts_c<-read.csv(table, header=TRUE)

vcf<-paste("lofreq_consensus_", samplename, sep="")
vcf<-paste(vcf, ".vcf", sep="")
vcf<-paste(outputdir, vcf, sep="/")
...
## rename ntcounts_r column headers
columns<-c("pos","ref","reads_all","mismatches","deletions","insertions","A","C","T","G")
...
csvfile<-paste(outputsample,"_mutational_spectrum.csv",sep="")
...
csvfile<-paste(outputsample,"_mutational_spectrum_frequencies.csv",sep="")
